...
<div>
    ...
    <br>
    <div v-transfer-dom>
        <confirm v-model="show3" show-input :title="$t('Confirm deleting the item')" :input-attrs="{type: 'number'}" @on-cancel="onCancel" @on-confirm="onConfirm" @on-show="onShow" @on-hide="onHide">
        </confirm>
    </div>
    <br>
    <div v-transfer-dom>
        <confirm v-model="show5" show-input ref="confirm5" :title="$t('Confirm deleting the item')" @on-cancel="onCancel" @on-confirm="onConfirm5" @on-show="onShow5" @on-hide="onHide">
        </confirm>
    </div>
    <br>
    <div v-transfer-dom>
        <confirm v-model="show2" :title="$t('Confirm deleting the item')" theme="android" @on-cancel="onCancel" @on-confirm="onConfirm" @on-show="onShow" @on-hide="onHide">
            <p style="text-align:center;">{{ $t('I miss u sunyi') }}</p>
        </confirm>
    </div>
    <br>
    <div v-transfer-dom>
        <confirm v-model="show4" :close-on-confirm="false" :title="$t('Confirm deleting the item')" @on-confirm="onConfirm4">
            <p style="text-align:center;">{{ $t('Are you sure?') }}</p>
        </confirm>
    </div>
    <br>
    <div v-transfer-dom>
        <confirm v-model="show6" :show-cancel-button="false" :title="$t('Confirm deleting the item')" @on-confirm="onConfirm">
            <p style="text-align:center;">{{ $t('Are you sure?') }}</p>
        </confirm>
    </div>
    <br>
