...
const fs = require('fs')
const pagesPath = path.resolve(__dirname, '../src/pages_list.json')
const glob = require('glob')
const match = path.join(__dirname, '../src/pages/*/*.vue')
    ...
    plugins: [
        ...
        },
        {
            ...
            fn: function(source) {
                ...
                // 这一步是为了生成自动生成 某些组件的 _index.vue 文件
                // 如果没有，则不会处理
                const listDir = glob.sync(match).filter(one => {
                    ...
                    const code = fs.readFileSync(path.join(__dirname, `../src/pages/${component}.vue`), 'utf-8')
                ...
                for (let dir in dirs) {
                    ...
                    `
                    fs.writeFileSync(path.join(__dirname, `../src/pages/${dir}/_index.vue`), file)
                ...
                list.forEach(one => {
                    let filename = one
                    // 将 大写 模块名转为小写的路径名： Home#/ 转为 /component/home#/
                    // 其是为了对应 路由中 component
                    ...
                    // console.log('---argv.env.include:', argv.env.include)
                ...
                str.push(`{
                path: '*',
                component: () => import('./pages/NotFoundComponent.vue').then(m => m.default)
                }`)
