...
function render(files, tag) {
  ...
  files.forEach(function (file) {
    ...
    if (!tag && item.icon && item.name) {
      const isDir = fs.existsSync(path.join(__dirname, `../src/pages/${item.name}/`))
      item.hasHome = isDir
...
<!--${t('该文件为自动生成，请不要修改')}-->
  \n## ${tag}`
    ...
    components.forEach(function (one) {
      ...
      docs += `\n&nbsp;&nbsp;<span style="color: #999;font-size:12px;"><a href="#" router-link="/zh-CN/pages/${(one.name || name).replace('-item', '')}">${t("进入demo页面", lang)}</a></span>`
...
`
  ...
  // docs += `\n<div id="play-${one.importName}" class="component-play-box"><a class="vux-demo-link" href="#" router-link="/zh-CN/pages/${one.name}">进入demo页面</a></div>\n`
  docs += `\n`
    // docs += `\n\n<span class="vux-props-title">Demo</span>\n`

  if (one.name || name) {
    if (one.hideDemo !== true) {
      docs += `\n<a class="vux-demo-link" href="#" router-link="/zh-CN/pages/${(one.name || name).replace('-item', '')}">${t("进入demo页面")}</a>\n`
    }
  }
