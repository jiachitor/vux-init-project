...
components.forEach((file) => {
  ...
  // demo 源码
  const demoPath = path.join(__dirname, `../src/pages/${importName}.vue`)
  let demoCode = ''
  try {
...
export default {
  ...
  langs.forEach(lang => {
    ...
    let demosDir = getPath(`../src/pages/${componentName}/`)
    if (fs.existsSync(demosDir)) {
      const list = glob.sync(demosDir + '/*.vue').filter(one => !one.includes('_index.vue'))
      .map(one => one.replace(getPath(`../src/pages/`), ''))
      .map(one => {
        const route = map[one.replace('.vue', '')]
        const code = fs.readFileSync(getPath(`../src/pages/`) + one, 'utf-8')
    ...
    let str = `
  <template>
    <div class="component-box" :class="pages.length ? 'components-with-toc' : ''">

      <div :class="pages.length ? 'component-header-v2' : 'component-header-v1'">
        ...
        <div class="title-box">
          ...
          <p class="component-extra-links">
            ...
            <a v-if="!pages.length" href="https://github.com/airyland/vux/blob/v2/src/demos/${importName}.vue" target="_blank" @click.prevent="showSourceCode">${t('demo source code', lang)}</a>
        ...
        <div v-if="!pages.length" class="component-demo" style="width:377px;height:600px;display:inline-block;border:1px solid #ececec;border-radius:5px;overflow:hidden;z-index:2500;">
      ...
      <a v-if="pages.length" class="anchor" id="examples">Examples</a>
      <br/>
      <template v-if="pages.length" v-for="demo in pages">
        <a class="anchor" :id="'examples:' + demo.title">{{ demo.title }}</a>
        <br/>
        <div class="demo-header">
          <span class="demo-title">{{ demo.title }}</span>
        </div>
        <div class="pages" :style="{height: demo.height}">
            ...
            <div v-if="demo.height === demo.oriHeight" class="demo-code-masker" @click="demo.height='auto'">
              <div>
                  ...
                  <span>
                    ...
                    Show code
                  <br/>
                  <!--<span class="demo-path">src/pages/{{ demo.file }}.vue</span>-->
                </span>
              </div>
            </div>
